...
`consistent-go` 是一个小巧、并发安全的进程内路由库，适用于稳定任务分配、分布式缓存路由、会话亲和负载均衡和应用管理的存储路由。它把虚拟节点哈希环、固定分区表和可配置负载上限组合在一起，让拓扑变更可观测，也更容易转化为实际迁移计划。
...
| 副本规划 | 有序且不重复的相邻候选成员 |
| 应用层分片路由 | 将 shard key 映射到固定分区和数据库实例 |
...
> **数据库能力边界：** `consistent-go` 只计算分片所有权，不负责拆分表或集合、搬迁数据、维护副本、协调跨分片事务，也不能替代 MongoDB Sharded Cluster 等原生分片系统。它适用于应用已经承担这些职责的场景，尤其是“租户到数据库”或“租户到独立数据库集群”的路由。
...
场景测试涵盖应用层分片路由、缓存故障转移、会话亲和负载均衡、并发成员变更和长时间稳定性。完整结构参见[测试指南](docs/testing-guide.md)。
